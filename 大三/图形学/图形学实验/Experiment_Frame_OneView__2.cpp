...
// Forward declaration of RoundFloat function
...
void CExperiment_Frame_OneView::DDA(int X0, int Y0, int X1, int Y1)
{
    ...
    // Determine the number of steps needed-确定最大位移方向
...
void CExperiment_Frame_OneView::Mid_Bresenham(int X0, int Y0, int X1, int Y1)
{
    ...
    // Determine the increments for x and y 因为x0可能>x1，y0可能大于y1
    ...
    if (dx >= dy) // Slope <= 1 确定最大位移方向，斜率绝对值小于1
    {
        decision = dx- 2 * dy; //计算D0
        Y = Y0;
        ...
        for (X = X0; X != X1; X += Xincrement)
        {
            ...
            if (decision < 0)//Di<0,y在方向上加1
            {
                Y += Yincrement;
                decision += 2 * dx;//根据Di和Di+1的关系求得Di+1
            }
            decision -= 2 * dy;
        }
